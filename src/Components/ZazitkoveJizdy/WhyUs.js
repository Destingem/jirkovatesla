...
function FeaturesCard({icon, title, children, device}){
  let icon2 = icon.data ? icon.data :<BsFillLightningChargeFill /> 
    ...
    <div style={{backgroundColor: "#e21837", borderRadius: "60px",  display: "flex", maxWidth: "fit-content", padding: "1rem", color: "rgb(227 227 227)"}}>{icon2}</div>
...
{whyUs.items1[0].map((item, index)=> {
  return(
    <FeaturesCard device={device} icon={item.icon} title={item.title}>{item.label}</FeaturesCard>
  )
})}
